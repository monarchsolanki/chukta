...
- **Drafting prompts** receive only typed facts, templates, and seller-authored text that has already been approved. Untrusted spans never enter them (SR-06). Dispute investigation, in Phase 2, may read untrusted spans, but it outputs only a JSON assessment that code validates. `03` fixes each node's input contract, and `04` defines the retrieval profiles that enforce it.
